...
def precalculate_data():
    ...
    with open('../_answers_/aggregate_025_20.txt', 'r') as csvfile:
        rows = csv.reader(csvfile, delimiter=',')
        for row in rows:
    ...
    fig, ax = plt.subplots()
    fig.suptitle('F1-score, noise 0.25, average 2 modules by 20 tests')
    fig.set_size_inches(9, 5)
    ax.boxplot(my_dict1.values(), vert=0)
    ax.set_yticklabels(my_dict1.keys())
    plt.savefig('../_answers_/picture_noise_025_f1score.png')
    plt.show()

    fig, ax = plt.subplots()
    fig.suptitle('AUC ROC, noise 0.25, average 2 modules by 20 tests')
    fig.set_size_inches(9, 5)
    ax.boxplot(my_dict2.values(), vert=0)
    ax.set_yticklabels(my_dict2.keys())
    plt.savefig('../_answers_/picture_noise_025_auc_roc.png')
    plt.show()
