...
def solve():
    for i in range(10):
        folder = '../agg_big_05/'
        test_name = 'a' + str(60 + i + 1) + '_big_test_05'
...
def precalculate_data():
    ...
    with open('../_answers_/aggregate_big_05_20.txt', 'r') as csvfile:
        rows = csv.reader(csvfile, delimiter=',')
        for row in rows:
    ...
    fig, ax = plt.subplots()
    fig.suptitle('F1-score, noise 05, big graph, average 2 modules by 20 tests')
    fig.set_size_inches(9, 5)
    ax.boxplot(my_dict1.values(), vert=0)
    ax.set_yticklabels(my_dict1.keys())
    plt.savefig('../_answers_/picture_big_noise_05_f1score.png')
    plt.show()

    fig, ax = plt.subplots()
    fig.suptitle('AUC ROC, noise 05, big graph, average 2 modules by 20 tests')
    fig.set_size_inches(9, 5)
    ax.boxplot(my_dict2.values(), vert=0)
    ax.set_yticklabels(my_dict2.keys())
    plt.savefig('../_answers_/picture_big_noise_05_auc_roc.png')
    plt.show()
